  ...
  return (
    <div className={`flex flex-col gap-${isMobile ? 3 : 4}`}>
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm p-4">
        <SectionHeading icon="📦" title="Inventory" sub="Stock health across all products" badge={`${flaggedProducts.length} need attention`} badgeColor="bg-red-50 text-red-500" />
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3 mb-4">
          {invKpis.map((k) => (
            <div key={k.label} className="rounded-xl p-3 border-l-4 bg-muted" style={{ borderLeftColor: k.color }}>
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide mb-1">{k.label}</p>
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-muted text-muted-foreground font-bold text-[10px] uppercase tracking-wide">
                <th className="px-3 py-2 text-left">Product</th>
                <th className="px-3 py-2 text-left">SKU</th>
                <th className="px-3 py-2 text-left">Category</th>
                <th className="px-3 py-2 text-center">In Stock</th>
                <th className="px-3 py-2 text-center">Reorder At</th>
                <th className="px-3 py-2 text-left w-24">Level</th>
                <th className="px-3 py-2 text-left">Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {stockItems.map((item) => {
                const isCritical = item.qty <= item.reorder;
                const pct = Math.min(100, (item.qty / (item.reorder * 3)) * 100);
                const barColor = item.qty <= 5 ? '#ef4444' : item.qty <= 20 ? '#f59e0b' : '#22c55e';
                return (
                  <tr key={item.sku} className={isCritical ? 'bg-red-50/30' : ''}>
                    <td className="px-3 py-2.5 font-semibold text-foreground">{item.name}</td>
                    <td className="px-3 py-2.5 text-muted-foreground font-mono">{item.sku}</td>
                    <td className="px-3 py-2.5 text-muted-foreground">{item.cat}</td>
                    <td className={`px-3 py-2.5 text-center font-black ${isCritical ? 'text-red-600' : 'text-foreground'}`}>{item.qty}</td>
                    <td className="px-3 py-2.5 text-center text-muted-foreground">{item.reorder}</td>
                    <td className="px-3 py-2.5 w-24"><Bar pct={pct} color={barColor} /></td>
                    <td className="px-3 py-2.5">
                      <span className={`flex items-center gap-1 text-[10px] font-bold ${isCritical ? 'text-red-500' : 'text-green-600'}`}>
                        <StockDot qty={item.qty} />
                        {isCritical ? 'Reorder' : 'Healthy'}
                      </span>
                    </td>
                  </tr>
                );
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm p-4">
        <SectionHeading icon="🌐" title="E-Commerce" sub="Online channel performance this month" badge="SSLCommerz Live" badgeColor="bg-green-50 text-green-600" />
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-3">
          <div className="xl:col-span-5 grid grid-cols-1 gap-2">
            {ecomKpis.map((k) => (
              <div key={k.label} className="bg-muted rounded-xl p-3 flex items-center gap-3">
                <Sparkline data={k.spark} color={k.color} />
                <div className="flex-1">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide">{k.label}</p>
                  <p className="text-base font-black text-foreground">{k.value}</p>
                  <p className="text-[10px] text-green-600 font-semibold">▲ {k.delta}</p>
                </div>
              </div>
            ))}
          </div>
          <div className="xl:col-span-7">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-bold text-muted-foreground">Recent Online Orders</p>
            ...
            <div className="space-y-1.5">
              {orders.slice(0, 4).map((o) => {
                ...
                return (
                  <div key={o.id} className="flex items-center justify-between bg-muted rounded-xl px-3 py-2">
                    <p className="text-xs font-bold text-primary w-24 shrink-0">{o.id}</p>
                    <p className="text-[11px] text-muted-foreground flex-1">{o.customer}</p>
                    <p className="text-xs font-bold text-foreground mr-3">{formatCurrency(o.amount)}</p>
