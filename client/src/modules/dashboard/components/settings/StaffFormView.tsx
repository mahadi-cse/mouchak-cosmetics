...
export default function StaffFormView({ userId, onBack }: Props) {
    ...
    <div>
      ...
      {tab === 'info' && (
        <div className="grid grid-cols-2 gap-3">
          <div><label className={lc}>First Name <span className="text-red-500">*</span></label><input required value={form.firstName} onChange={(e) => setForm({ ...form, firstName: e.target.value })} className={`${ic} transition-colors ${!form.firstName.trim() ? 'border-red-400 bg-red-50' : ''}`} placeholder="First name" /></div>
          ...
          <div><label className={lc}>Email <span className="text-red-500">*</span></label><input required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className={`${ic} transition-colors ${!isEdit && !form.email.trim() ? 'border-red-400 bg-red-50' : ''}`} placeholder="user@company.com" disabled={isEdit} style={{ opacity: isEdit ? 0.6 : 1 }} /></div>
          ...
          <div><label className={lc}>{isEdit ? 'New Password' : 'Password '} {!isEdit && <span className="text-red-500">*</span>}</label><input required={!isEdit} type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className={`${ic} transition-colors ${!isEdit && !form.password.trim() ? 'border-red-400 bg-red-50' : ''}`} placeholder="••••••••" /></div>
          <div><label className={lc}>User Type <span className="text-red-500">*</span></label><select required value={form.typeId} onChange={(e) => setForm({ ...form, typeId: e.target.value })} className={`${sc} transition-colors ${!form.typeId ? 'border-red-400 bg-red-50' : ''}`}><option value="">— Select —</option>{userTypes.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}</select></div>
